...
int main(int argc, char* argv[])
{
    ...
    if (is_ok && WSInitialize()) {
        printf("Establish %d connections\n", clients);
        int _count = 0;
        SOCKET sockets[1024];
        for (int i = 0; i < clients; ++i) {
            sockets[i] = CreateSocket(TCP);
            if (sockets[i] != INVALID_SOCKET) {
                SetReceiveTimeout(sockets[i], RECEIVE_TIMEOUT_INTERVAL);

                ADDRESS server = CreateSocketAddress(server_ip, server_port);
            
                do {
                    if (EstablishConnection(sockets[i], server)) {
                        //printf("[%s] Ready to communicate...\n", INFO_FLAGS);
                        //if (++_count % 5 == 0)
                        //    printf("Number of success clients: %d\n", _count);
                    }
                } while (0);
        ...
        scanf_s("%d", &clients, 4);
        for (int i = 0; i < clients; ++i)
            CloseSocket(sockets[i], CLOSE_SAFELY, SD_BOTH);
        printf("Close clients sockets.....\n");
        // Test 2
        printf("Enter number of clients: ");
        scanf_s("%d", &clients, 4);
        printf("Establish %d connections and send USER requests\n", clients);
        _count = 0;
        for (int i = 0; i < clients; ++i) {
            sockets[i] = CreateSocket(TCP);
            if (sockets[i] != INVALID_SOCKET) {
                ...
                do {
                    if (EstablishConnection(sockets[i], server)) {
                        char request[] = "USER admin";
                        int ret = Run(sockets[i], request);
                        //printf("[%s] Ready to communicate...\n", INFO_FLAGS);
                        if (ret == 1) {
                            if (++_count % 5 == 0)
                                printf("Number of success send clients: %d\n", _count);
                        }
                    }
                } while (0);
